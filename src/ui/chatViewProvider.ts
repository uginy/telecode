...
export class ChatViewProvider implements vscode.WebviewViewProvider {
			...
			if (
				...
				const maxSteps =
					Number.isFinite(maxStepsRaw) && maxStepsRaw > 0
						...
						: 100;
				const logMaxCharsRaw =
					typeof raw.logMaxChars === "number"
						? raw.logMaxChars
						: Number(raw.logMaxChars);
				const logMaxChars =
				...
				this.commandEmitter.fire({
					...
					settings: {
						...
						maxSteps,
						responseStyle:
							typeof raw.responseStyle === "string"
								? raw.responseStyle
								: "concise",
						...
						statusVerbosity:
							typeof raw.statusVerbosity === "string"
								...
								: "normal",
						safeModeProfile:
							typeof raw.safeModeProfile === "string"
								? raw.safeModeProfile
								: "balanced",
						...
						telegramBotToken:
							typeof raw.telegramBotToken === "string"
								...
								: "",
						telegramChatId:
							typeof raw.telegramChatId === "string" ? raw.telegramChatId : "",
						telegramApiRoot:
							typeof raw.telegramApiRoot === "string"
								? raw.telegramApiRoot
								: "",
						telegramForceIPv4: raw.telegramForceIPv4 !== false,
						whatsappEnabled: raw.whatsappEnabled === true,
						whatsappSessionPath:
							typeof raw.whatsappSessionPath === "string"
								? raw.whatsappSessionPath
								: "~/.telecode-ai/whatsapp-session.json",
						whatsappAllowSelfCommands: raw.whatsappAllowSelfCommands !== false,
						whatsappAccessMode:
							raw.whatsappAccessMode === "allowlist" ||
							raw.whatsappAccessMode === "all" ||
							raw.whatsappAccessMode === "self"
								? raw.whatsappAccessMode
								: "self",
						whatsappAllowedPhones:
							typeof raw.whatsappAllowedPhones === "string"
								? raw.whatsappAllowedPhones
								: "",
					},
				});
			}
	...
	appendOutput(text: string): void {
		const maxChars = this.logMaxChars;
	...
	setSettings(settings: ChatViewSettings): void {
		...
		this.logMaxChars =
			settings.logMaxChars > 0
				? settings.logMaxChars
				: DEFAULT_OUTPUT_MAX_CHARS;
		this.post({ type: "settings", settings });
		i18n.setLanguage(
			resolveUiLanguage(settings.uiLanguage as "ru" | "en" | "auto"),
		);
